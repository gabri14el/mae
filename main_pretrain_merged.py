...
def get_args_parser():
    ...
    parser.add_argument('--data_path', default='/datasets01/imagenet_full_size/061417/', type=str,
                        help='dataset path')
    parser.add_argument('--hard_augmentation', action='store_true', default=False)
    ...
    parser.add_argument('--bt_normalization', default='batch', choices=['batch', 'layer'], type=str)
    parser.add_argument('--bt_strong_augmentation', action='store_true', default=False)
    parser.add_argument('--bt_loss_coef', default=0.001, type=float)
    ...
    #knn evaluation
    parser.add_argument('--knn_eval', action='store_true', default=False)
    parser.add_argument('--knn_k', default=5, type=int)
    parser.add_argument('--knn_dataset', default=None, type=str)
...
def main(args):
    ...
    bt_normalization = torch.nn.BatchNorm1d if args.bt_normalization == 'batch' else torch.nn.LayerNorm
    ...
    dataset_train = datasets.ImageFolder(os.path.join(args.data_path, 'train'), transform=Transform(args))
    print(dataset_train)
    ...
    if args.knn_eval:
        ...
        dataset_eval_validation = datasets.ImageFolder(os.path.join(args.knn_dataset, 'val'), transform=transform_eval)
    ...
    data_loader_eval_validation = None
    if args.knn_eval:
        data_loader_eval_train = torch.utils.data.DataLoader(
            dataset_eval_train, batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=args.pin_mem,
            drop_last=False,
        )
        data_loader_eval_validation = torch.utils.data.DataLoader(
            dataset_eval_validation, batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=args.pin_mem,
            drop_last=False,
        )
